...
void matmul(float* A, float* B, float* bias, float* out, int M, int K, int N){
    ...
    for(int m = 0; m<M; m++){
        for(int n = 0; n<N; n++){
            ...
            cout << "out[m*N + n] = " << "m * N + n = " << m << "*" << N << "+" << n << "=" << out[m*N+n];
        }
    }
...
// out[m*N + n] = m * N + n = 0*3+0=31val : 2 m:0 n:1 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 0*3+1=38val : 3 m:0 n:2 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 0*3+2=45val : 1 m:1 n:0 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 1*3+0=67val : 2 m:1 n:1 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 1*3+1=83val : 3 m:1 n:2 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 1*3+2=99val : 1 m:2 n:0 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 2*3+0=103val : 2 m:2 n:1 k:0 this is before updating the inner loop
...
// out[m*N + n] = m * N + n = 2*3+1=128val : 3 m:2 n:2 k:0 this is before updating the inner loop
